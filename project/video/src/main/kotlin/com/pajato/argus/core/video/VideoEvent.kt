...
sealed class VideoEvent {
    ...
    abstract fun load(videoIdMap: MutableMap<Long, CoreVideo>)
...
class ArchiveEvent(val videoId: String = "", val state: String = "") : VideoEvent() {
    ...
    override fun load(videoIdMap: MutableMap<Long, CoreVideo>) {
        val videoIdAsLong = videoId.toLongOrNull() ?: return
        val video = videoIdMap[videoIdAsLong] ?: return
...
class RegisterEvent(val videoId: String = "", val videoName: String = "") : VideoEvent() {
    ...
    override fun load(videoIdMap: MutableMap<Long, CoreVideo>) {
        ...
        val video = CoreVideo(videoIdAsLong, data)
        videoIdMap[videoIdAsLong] = video
    }
...
class UpdateEvent(
    ...
    override fun load(videoIdMap: MutableMap<Long, CoreVideo>) {
        ...
        video.updateForLoad(mutableSetOf(attribute), updateType)
    }
